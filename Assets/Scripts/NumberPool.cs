...
public class NumberPool : MonoBehaviour
{
    ...
    [SerializeField]
    List<GameObject> pooledNumbers = new List<GameObject>();

    public GameObject
    GetPooledObject(int numberToGet, Transform targetNode, bool wantActive)
    {
        for (int i = 0; i < pooledNumbers.Count; i++)
        {
            ...
            if (
                currentNumber.name.Equals(numberToGet.ToString()) &&
                !currentNumber.GetComponent<Number>().isUsing
            )
            {
        ...
        GameObject newBlock =
            Instantiate(Resources.Load("Nodes/" + numberToGet.ToString()),
            targetNode) as
            GameObject;
    ...
    public void GetPooledObjectOnClick(
        int index,
        Transform targetNode,
        bool wantActive
    )
    {
        if (index < 0 || index >= pooledNumbers.Count) return;

        GameObject currentNumber = pooledNumbers[index];

        if (!currentNumber.GetComponent<Number>().isUsing)
        {
            currentNumber.GetComponent<Number>().isUsing = true;
            currentNumber.SetActive (wantActive);
            currentNumber.transform.SetParent (targetNode);
